			...
			<div id="content">
				...
				<div id="filters" class="row-fluid">
					...
					<div class="span8 text-right">
						<form action="<c:url value="/admin/testcase/all-testcases" />" method="GET"
							class="row-fluid">
							 <c:if test="${myProfile.role.roleType == 'admin'}">                        
							<div class="span4">
								<div class="control-group">
									<input id="userName" name="userName" class="span12" type="text"
										placeholder="案例提交者" value="${userName}" />
								</div>
								<!-- .control-group -->
							</div>
							</c:if>
						...
						<c:forEach var="testcase" items="${testcases}">
							<tr data-value="${testcase.testcaseId}">
								...
								<td class="problem-id"><a
									href="<c:url value="/admin/testcase/edit-testcase/${testcase.testcaseId}" />">${testcase.testcaseId}</a>
								</td>
								...
								<td class="problem-categories"><c:choose>
										...
										<c:otherwise>
											<c:forEach var="testcaseCategory"
												...
												varStatus="loop">
												<%--  <a href="<c:url value="/admin/testcase/all-testcases?testcaseCategory=${testcaseCategory.categoryType}" />">
				...
				<div id="pagination" class="pagination pagination-centered">
					...
					<c:set var="baseUrl"
						value="/admin/testcase/all-testcases?keyword=${keyword}&categoryType=${selectedCategory}" />
					<ul>
						<li
	...
	<script type="text/javascript">
		function doDeleteTestcasesAction(testcases) {
			...
			$
					.ajax({
						type : 'POST',
						url : '<c:url value="/admin/testcase/deleteTestcases.action" />',
						data : postData,
						dataType : 'JSON',
